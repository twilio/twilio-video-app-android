...
fun enterYourName(name: String) {
    onView(withId(R.id.`@+id/name`)).perform(clearText(), typeText(name))
}
...
fun assertLoadingIndicatorIsDisplayed() {
    onView(withId(R.id.`@+id/progress_bar`)).check(matches(isDisplayed()))
}

fun assertLoadingIndicatorIsNotDisplayed() {
    onView(withId(R.id.`@+id/progress_bar`)).check(matches(not(isDisplayed())))
}

fun assertLoginButtonIsEnabled() {
    onView(withId(R.id.`@+id/login`)).check(matches(isEnabled()))
}

fun assertLoginButtonIsDisabled() {
    onView(withId(R.id.`@+id/login`)).check(matches(not(isEnabled())))
}
...
fun assertThatPasscodeErrorIsDisabled() {
    onView(withId(R.id.`@+id/passcode_input`)).check(matches(not(isTextInputLayoutError())))
}
