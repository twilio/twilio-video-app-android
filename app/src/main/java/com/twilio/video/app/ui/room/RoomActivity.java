...
public class RoomActivity extends BaseActivity {
  ...
  private final ScreenCapturer.Listener screenCapturerListener =
      new ScreenCapturer.Listener() {
        ...
        public void onScreenCaptureError(String errorDescription) {
          ...
          stopScreenCapture();
          Toast.makeText(RoomActivity.this, R.string.screen_capture_error, Toast.LENGTH_LONG)
              .show();
        }
  ...
  public void onActivityResult(int requestCode, int resultCode, Intent data) {
    if (requestCode == MEDIA_PROJECTION_REQUEST_CODE) {
      if (resultCode != Activity.RESULT_OK) {
        Toast.makeText(this, R.string.screen_capture_permission_not_granted, Toast.LENGTH_LONG)
            .show();
  ...
  void connectButtonClick() {
    ...
    Topology topology =
        Topology.fromString(
            sharedPreferences.getString(Preferences.TOPOLOGY, Preferences.TOPOLOGY_DEFAULT));

    Single<Room> connection =
        updateEnv()
            .andThen(tokenService.getToken(displayName, topology))
            .flatMap(
                new Function<String, SingleSource<? extends Room>>() {
                  @Override
  ...
  void toggleLocalVideo() {
    ...
    if (cameraVideoTrack == null) {
      ...
      cameraVideoTrack =
          LocalVideoTrack.create(this, true, cameraCapturer.getVideoCapturer(), videoConstraints);
      if (localParticipant != null) {
  ...
  private void obtainVideoConstraints() {
    ...
    int maxVideoDim =
        sharedPreferences.getInt(Preferences.MAX_VIDEO_DIMENSIONS, videoDimensions.length - 1);
  ...
  private void setupLocalVideoTrack() {
    ...
    if (cameraVideoTrack != null) {
      ...
      Snackbar.make(
              primaryVideoView, R.string.failed_to_add_camera_video_track, Snackbar.LENGTH_LONG)
          .show();
    }
  }
  ...
  private void startScreenCapture() {
    ...
    if (screenVideoTrack != null) {
      ...
      Snackbar.make(
              primaryVideoView, R.string.failed_to_add_screen_video_track, Snackbar.LENGTH_LONG)
    ...
    return Completable.fromAction(
        new Action() {
          ...
          public void run() throws Exception {
            ...
            Env.set(
                RoomActivity.this, EnvUtil.TWILIO_ENV_KEY, nativeEnvironmentVariableValue, true);
          }
        });
  }
        ...
        new Callable<Room>() {
          ...
          public Room call() throws Exception {
            ...
            boolean enableInsights =
                sharedPreferences.getBoolean(
                    Preferences.ENABLE_INSIGHTS, Preferences.ENABLE_INSIGHTS_DEFAULT);

            VideoCodec preferedVideoCodec =
                getVideoCodecPreference(Preferences.VIDEO_CODEC, Preferences.VIDEO_CODEC_DEFAULT);

            AudioCodec preferredAudioCodec =
                getAudioCodecPreference(Preferences.AUDIO_CODEC, Preferences.AUDIO_CODEC_DEFAULT);

            ConnectOptions.Builder connectOptionsBuilder =
                new ConnectOptions.Builder(token).roomName(roomName).enableInsights(enableInsights);

            int maxVideoBitrate =
                sharedPreferences.getInt(
                    Preferences.MAX_VIDEO_BITRATE, Preferences.MAX_VIDEO_BITRATE_DEFAULT);

            int maxAudioBitrate =
                sharedPreferences.getInt(
                    Preferences.MAX_AUDIO_BITRATE, Preferences.MAX_AUDIO_BITRATE_DEFAULT);
            ...
            connectOptionsBuilder.preferVideoCodecs(Collections.singletonList(preferedVideoCodec));

            connectOptionsBuilder.preferAudioCodecs(Collections.singletonList(preferredAudioCodec));

            connectOptionsBuilder.encodingParameters(encodingParameters);

            room = Video.connect(RoomActivity.this, connectOptionsBuilder.build(), roomListener());
            return room;
          }
        });
  }
  ...
  private void renderItemAsPrimary(ParticipantController.Item item) {
    ...
    if (old != null) {
      ...
      if (old.sid.equals(localParticipantSid)) {
        ...
        int state =
            old.videoTrack == null ? ParticipantView.State.NO_VIDEO : ParticipantView.State.VIDEO;
  ...
  private void updateStatsUI(boolean enabled) {
    if (enabled) {
      if (room != null && room.getRemoteParticipants().size() > 0) {
        ...
        statsDisabledDescTextView.setText(getString(R.string.stats_description_media_not_shared));
  ...
  /**
   * Provides participant thumb click listener. On thumb click appropriate video track is being send
   * to primary view. If local camera track becomes primary, it should just change it state to
   * SELECTED state, if remote particpant track is going to be primary - thumb is removed.
   *
  ...
  private class ParticipantListener implements RemoteParticipant.Listener {
    ...
    public void onVideoTrackSubscribed(
      ...
      if (primary != null
        ...
        participantController.addOrUpdateThumb(
            remoteParticipant.getSid(), remoteParticipant.getIdentity(), null, remoteVideoTrack);
      }
    }
    ...
    public void onVideoTrackUnsubscribed(
      ...
      if (primary != null
        ...
        participantController.removeOrEmptyThumb(
            remoteParticipant.getSid(), remoteParticipant.getIdentity(), remoteVideoTrack);
      }
    }
    ...
    public void onDataTrackSubscriptionFailed(
      ...
      Timber.w(
          ...
          twilioException.getMessage());
      // TODO: Need design
      Snackbar.make(primaryVideoView, "onDataTrackSubscriptionFailed", Snackbar.LENGTH_LONG).show();
    }
