  ...
  TokenService providesTokenService(
    ...
    return new TokenService() {
      ...
      public Single<String> getToken(final String identity, final Topology topology) {
        final String env =
            sharedPreferences.getString(Preferences.ENVIRONMENT, Preferences.ENVIRONMENT_DEFAULT);

        return videoAppService
            .getConfiguration(env)
            .flatMap(
                new Function<VideoConfiguration, SingleSource<? extends String>>() {
                  @Override
                  public SingleSource<? extends String> apply(
                      @NonNull VideoConfiguration videoConfiguration) throws Exception {
