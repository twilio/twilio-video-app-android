  ...
  public static VideoRoom createRoom(
    ...
    do {
      try {
        videoRoom =
            videoApiService.createRoom(
                authorization, name, type, enableTurn, enableRecording, videoCodecs);
      } catch (RetrofitError createRoomError) {
        Log.e(
            "VideoApiUtils", String.format("RetrofitError: %s", createRoomError.getKind().name()));
        ...
        try {
          ...
          Log.e("VideoApiUtils", String.format("RetrofitError: %s", getRoomError.getKind().name()));
        }
      }
    ...
    assertNotNull(
        String.format("Failed to create a Room after %s attempts", String.valueOf(MAX_RETRIES)),
        videoRoom);
