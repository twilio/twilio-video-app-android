...
void AndroidVideoCapturerJni::Start(const cricket::VideoFormat& capture_format,
...
bool AndroidVideoCapturerJni::IsScreencast() {
...
AndroidVideoCapturerJni::GetSupportedFormats() {
    ...
    for (int i = 0; i < size; ++i) {
        ...
        const int frame_interval = cricket::VideoFormat::FpsToInterval(
                (webrtc_jni::GetIntField(jni, j_framerate, j_max_framerate_field) + 999) / 1000);
...
JNIEnv* AndroidVideoCapturerJni::jni() { return webrtc_jni::AttachCurrentThreadIfNeeded(); }
