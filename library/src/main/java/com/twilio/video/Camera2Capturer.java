...
public class Camera2Capturer implements VideoCapturer {
        ...
        public void onCameraSwitchError(final String errorMessage) {
          ...
          handler.post(
              new Runnable() {
                @Override
                public void run() {
                  listener.onError(new Exception(Exception.CAMERA_SWITCH_FAILED, errorMessage));
                }
              });
        }
      };
   ...
   *
   * @param context application context
   * @param cameraId unique identifier of the camera device to open that must be specified in {@link
   ...
   * {@link android.hardware.camera2.CameraCharacteristics}, so can vary based on a device's camera
   * capabilities.
   *
  ...
  public synchronized List<VideoFormat> getSupportedFormats() {
    ...
    if (supportedFormats == null) {
      supportedFormats = convertToVideoFormats(camera2Enumerator.getSupportedFormats(cameraId));
  ...
  public synchronized void switchCamera(@NonNull final String newCameraId) {
    ...
    Preconditions.checkArgument(
        !newCameraId.equals(cameraId), "Camera ID must be different " + "from current camera ID");
    Preconditions.checkArgument(
