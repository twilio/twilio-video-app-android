...
public class CameraCapturer implements VideoCapturer {
  ...
  private final org.webrtc.VideoCapturer.CapturerObserver observerAdapter =
      new org.webrtc.VideoCapturer.CapturerObserver() {
        ...
        public void onByteBufferFrameCaptured(
          ...
          VideoFrame frame =
              new VideoFrame(
                  bytes, frameDimensions, VideoFrame.RotationAngle.fromInt(rotation), timestamp);
  ...
  public CameraCapturer(
      @NonNull Context context, @NonNull CameraSource cameraSource, @Nullable Listener listener) {
  ...
  /**
   * Switches the current {@link CameraSource}. This method can be invoked while capturing frames or
   * not.
   */
  public synchronized void switchCamera() {
    ...
    if (!nextCameraSourceSupported) {
      logger.w(String.format("Cannot switch to unsupported camera source %s", nextCameraSource));
      return;
    }
   ...
   * CameraParameterUpdater#apply(Camera.Parameters)}. Any changes to the parameters will be applied
   * after the invocation of this callback. This method can be invoked while capturing frames or
   * not.
   *
   ...
   * </code></pre>
   *
   * @param cameraParameterUpdater camera parameter updater that receives current camera parameters
   *     for modification.
   ...
   *     taken event.
   * @return true if picture was scheduled to be taken or false if a picture is pending or could not
   *     be scheduled.
   */
  ...
  private void updateCameraParametersOnCameraThread(
      final CameraParameterUpdater cameraParameterUpdater) {
    synchronized (stateLock) {
      if (state == State.RUNNING) {
  ...
  private void takePictureOnCameraThread(
      final Handler pictureListenerHandler, final PictureListener pictureListener) {
    synchronized (stateLock) {
      if (state == State.RUNNING) {
        // Validate execution on camera thread
        camera1Session.checkIsOnCameraThread();
        ...
        camera1Session.camera.takePicture(
            ...
            new android.hardware.Camera.PictureCallback() {
              @Override
              public void onPictureTaken(byte[] pictureData, android.hardware.Camera camera) {
