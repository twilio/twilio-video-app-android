...
public class VideoTopologyParameterizedTest extends BaseVideoTest {
  ...
  public void canConnectWithInsightsDisabled() throws InterruptedException {
    roomListener.onConnectedLatch = new CountDownLatch(1);
    ConnectOptions connectOptions = new ConnectOptions.Builder(token).enableInsights(false).build();
    room = Video.connect(mediaTestActivity, connectOptions, roomListener);
    assertTrue(roomListener.onConnectedLatch.await(20, TimeUnit.SECONDS));
    assertEquals(room.getSid(), room.getName());
  }
  ...
  public void connect_shouldAllowDataTracks() throws InterruptedException {
    ...
    ConnectOptions connectOptions =
        new ConnectOptions.Builder(token).roomName(roomName).dataTracks(localDataTrackList).build();
    room = Video.connect(mediaTestActivity, connectOptions, roomListener);
    assertTrue(roomListener.onConnectedLatch.await(20, TimeUnit.SECONDS));
    LocalParticipant localParticipant = room.getLocalParticipant();
  ...
  public void connect_shouldAllowLocalVideoTrackToBeReleasedWhileConnecting()
    ...
    ConnectOptions connectOptions =
        new ConnectOptions.Builder(token).roomName(roomName).videoTracks(localVideoTracks).build();
    room = Video.connect(mediaTestActivity, connectOptions, roomListener);
    ...
    assertTrue(
        room.getLocalParticipant().getLocalVideoTracks().get(0).getLocalVideoTrack().isReleased());
  }
  ...
  public void connect_shouldAllowLocalDataTrackToBeReleasedWhileConnecting()
    ...
    ConnectOptions connectOptions =
        new ConnectOptions.Builder(token).roomName(roomName).dataTracks(localDataTracks).build();
    room = Video.connect(mediaTestActivity, connectOptions, roomListener);
    ...
    assertEquals(
        localDataTrack, room.getLocalParticipant().getLocalDataTracks().get(0).getLocalDataTrack());
    assertTrue(
        room.getLocalParticipant().getLocalDataTracks().get(0).getLocalDataTrack().isReleased());
  }
  ...
  public void connect_shouldAllowLocalAudioTrackToBeReleasedWhileConnecting()
    ...
    ConnectOptions connectOptions =
        new ConnectOptions.Builder(token).roomName(roomName).audioTracks(localAudioTracks).build();
    room = Video.connect(mediaTestActivity, connectOptions, roomListener);
    ...
    assertTrue(
        room.getLocalParticipant().getLocalAudioTracks().get(0).getLocalAudioTrack().isReleased());
  }
