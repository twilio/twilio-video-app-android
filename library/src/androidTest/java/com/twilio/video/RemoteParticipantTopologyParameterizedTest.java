...
public class RemoteParticipantTopologyParameterizedTest extends BaseParticipantTest {
  ...
  public void participantCanConnect() throws InterruptedException {
    ...
    ConnectOptions connectOptions = new ConnectOptions.Builder(tokenOne).roomName(roomName).build();
  ...
  public void shouldReceiveTrackEvents() throws InterruptedException {
    ...
    assertTrue(participantListener.onUnsubscribedFromAudioTrackLatch.await(20, TimeUnit.SECONDS));
    ...
    bobLocalVideoTrack = LocalVideoTrack.create(mediaTestActivity, true, new FakeVideoCapturer());
    ...
    assertTrue(participantListener.onUnsubscribedFromDataTrackLatch.await(20, TimeUnit.SECONDS));
    ...
    assertArrayEquals(
        expectedParticipantEvents.toArray(), participantListener.participantEvents.toArray());
  }
  ...
  public void shouldHaveTracksAfterDisconnected() throws InterruptedException {
    ...
    assertEquals(
        remoteAudioTrackPublications.get(0), remoteParticipant.getRemoteAudioTracks().get(0));
    assertEquals(audioTrackPublications.get(0), remoteParticipant.getAudioTracks().get(0));
    assertEquals(
        remoteVideoTrackPublications.get(0), remoteParticipant.getRemoteVideoTracks().get(0));
