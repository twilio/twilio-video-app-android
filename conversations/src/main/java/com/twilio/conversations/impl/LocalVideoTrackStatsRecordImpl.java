...
public class LocalVideoTrackStatsRecordImpl extends MediaTrackStatsRecordImpl
        implements LocalVideoTrackStatsRecord {
    ...
    public LocalVideoTrackStatsRecordImpl(CoreTrackStatsReport report) {
        super(report);
        ...
        int width = report.getIntValue(CoreTrackStatsReport.KeyEnum.FRAME_WIDTH_INPUT) < 0 ? 0 : report.getIntValue(CoreTrackStatsReport.KeyEnum.FRAME_WIDTH_INPUT);
        int height = report.getIntValue(CoreTrackStatsReport.KeyEnum.FRAME_HEIGHT_INPUT) < 0 ? 0 : report.getIntValue(CoreTrackStatsReport.KeyEnum.FRAME_HEIGHT_INPUT);
        captureDimensions = new VideoDimensions(width, height);
        // coerce the width and height to ensure they are non-negative values
        width = report.getIntValue(CoreTrackStatsReport.KeyEnum.FRAME_WIDTH_SENT) < 0 ? 0 : report.getIntValue(CoreTrackStatsReport.KeyEnum.FRAME_WIDTH_SENT);
        height = report.getIntValue(CoreTrackStatsReport.KeyEnum.FRAME_HEIGHT_SENT) < 0 ? 0 : report.getIntValue(CoreTrackStatsReport.KeyEnum.FRAME_HEIGHT_SENT);
